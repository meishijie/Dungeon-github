 ...
 **/
private enum State {
  KeyInput;       // キー入力待ち 等待操作
  InventoryInput; // インベントリの操作中 背包操作中
  PlayerAct;      // プレイヤーの行動 玩家的行动
  Firearm;        // 飛び道具 飞行道具
  Magicbullet;    // 魔法弾
  PlayerActEnd;   // プレイヤー行動終了 玩家行动结束
  EnemyRequestAI; // 敵のAI 敌人的AI
  Move;           // 移動
  EnemyActBegin;  // 敵の行動開始
  EnemyAct;       // 敵の行動
  EnemyActEnd;    // 敵の行動終了
  TurnEnd;        // ターン終了 一局结束
  NextFloor;      // 次のフロアに進むかどうか 下一个楼层是否前进
  NextFloorWait;  // 次のフロアに進む（完了待ち）下一个楼层 等待中
  NextFloorWarp;  // 次のフロアへワープ 向下一个楼层
  ShopOpen;       // ショップメニューを開く 打开店铺菜单
  ShopRoot;       // ショップルートメニュー 商店根
  ShopSell;       // ショップ(売却) 卖
  ShopBuy;        // ショップ(購入)买
...
class SeqMgr {
  ...
  // updateの戻り値 返回值
  public static inline var RET_NONE:Int = 0; // 何もなし 什么都没有
  public static inline var RET_GAMEOVER:Int = 1; // ゲームオーバー 游戏结束
  public static inline var RET_GAMECLEAR:Int = 2; // ゲームクリア 游戏清除
